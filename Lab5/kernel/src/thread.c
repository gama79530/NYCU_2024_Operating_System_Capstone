...
int create_task(uint64_t flag, uint8_t priority, task_routine_t routine, void *arg){
    disable_preemption();
    bool is_fork = (flag & FLAG_FORK) != 0;
    kernel_task_t *current_task = get_current_task();
    kernel_task_t *new_task = (kernel_task_t*)malloc(TASK_STACK_SIZE);
    if(new_task == NULL){
        return -1;
    }
    ...
    if(routine != NULL){
        ...
        new_task->user_stack = (uint64_t)malloc(THREAD_STACK_SIZE);  // allocate new user stack
    ...
    new_task->cpu_context.x21 = ((is_fork ? current_task->flag : flag) & FLAG_ENTER_USER_MODE);
    ...
    new_task->flag = is_fork ? current_task->flag : flag;
    new_task->pid = get_new_pid();
    new_task->state = RUNNING;
    new_task->priority = is_fork ? current_task->priority : (priority ? priority : current_task->priority);
...
int enter_user_mode(uint64_t pc){
    kernel_task_t *current_task = get_current_task();
    current_task->user_stack = (uint64_t)malloc(THREAD_STACK_SIZE);  // allocate new user stack
