...
// "cpio_base" is set by the default setting in "config.txt", but we can read the value from the device tree rather than hard coding it. 
...
void cpio_set_by_dtb_callback(uint32_t token, const char *name, const void *data, uint32_t len){
    ...
    if(token_little == FDT_PROP){
        if(!strcmp(name, "linux,initrd-start")){
            endian_exchange(&data_little, (uint32_t*)data, 4);
