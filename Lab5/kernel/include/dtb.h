...
typedef struct fdt_header{
    ...
    uint32_t size_dt_strings;   // length of the strings block
    uint32_t size_dt_struct;    // length of the structure block
} fdt_header_t;
...
/*  
1. All arguments are in big-endian format if they are considered as integers.
2. The callback function should perform both token and name comparisons and handle each separately.
3. if token is FDT_BEGIN_NODE   
    i. name is node name and the other arguments are not used.
4. if toker is FDT_PROP
    i.  name is property name
    ii. data is the property value
    iii.len is the length of the property’s value in bytes
*/ 
